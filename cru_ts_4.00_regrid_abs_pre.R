...
ncinfile <- "na10km_v2_cru_ts4.00.1901.2015.pre.anm3d.nc"
...
png(file=paste(out,"na10km_v2_cru_4.00.2015.anm.pre3d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-1000,-500,-200,0,50,100,200,500,1000,2000,3000)
png(file=paste(out,"na10km_v2_cru_4.00.2013.anm.pre4d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-1000,-500,-200,0,50,100,200,500,1000,2000,3000)
png(file=paste(out,"na10km_v2_cru_4.00.2015.abs.pre3d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
ptm <- proc.time() # timer
absfile <- paste0("na10km_v2_cru_ts4.00.",start_year,".",end_year,".pre.abs3d.nc")
...
ptm <- proc.time() # timer
absfile <- paste0("na10km_v2_cru_ts4.00.",start_year,".",end_year,".pre.abs4d.nc")
abs_ncfile <- paste(tspath,absfile,sep="")
