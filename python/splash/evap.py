    ...
    def calculate_daily_fluxes(self, sw, n, y=0, sf=1.0, tc=23.0):
        ...
        try:
            ...
            self.logger.info(
                ...
                "Celcius, and supply rate %f"
                % (n, self.solar.kN, self.solar.year, sf, tc, sw))

        # 3. Calculate water-to-energy conversion (econ), m^3/J------------
        ...
        # 5. Estimate daily equilibrium ET (eet_d), mm---------------------
        eet_d = (1e3) * econ * rn_d
        self.eet_d = eet_d
        self.logger.info("daily EET set to %f mm" % eet_d)
        
        # 6. Estimate daily potential ET (pet_d), mm-----------------------
        pet_d = (1.0 + kw)*eet_d
        self.pet_d = pet_d
        self.logger.info("daily PET set to %f mm" % pet_d)

        # 7. Calculate variable substitute (rx), (mm/hr)/(W/m^2)-----------
        rx = (3.6e6) * (1.0 + kw) * econ
        self.rx = rx
        self.logger.info("variable substitute, rx, set to %f" % rx)

        # 8. Calculate the intersection hour angle (hi), degrees-----------
        ...
        # 9. Estimate daily actual ET (aet_d), mm--------------------------
        aet_d = sw * hi * pir
