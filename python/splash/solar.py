...
# modification:
# ~~~~~~~~~~~~~
# Additional modifications by Damian Satterthwaite-Phillips
# 2018-07-17
import logging
...
class Solar:
    ...
    def calculate_daily_fluxes(self, n, y=0, sf=1.0, tc=23.0):
        ...
        # 2. Calculate number of days in year (kN), days-------------------
        if y == 0:
            kN = 365
            self.year = 2001
        elif y < 0:
            ...
            self.logger.info(
                ...
                "%f Celcius"
                % (n, kN, self.year, sf, tc))

        # 3. Calculate heliocentric longitudes (nu and lambda), degrees----
        # Berger (1978)
        ...
        # 4. Calculate distance factor (dr), unitless----------------------
        # Berger et al. (1993)
        kee = ke**2
        ...
        # 5. Calculate declination angle (delta), degrees------------------
        # Woolf (1968)
        ...
        # 6. Calculate variable substitutes (u and v), unitless------------
        ...
        # 7. Calculate the sunset hour angle (hs), degrees-----------------
        ...
        # 8. Calculate daily extraterrestrial solar radiation (ra_d), J/m^2
        ...
        self.tau = tau
        self.logger.info("base transmitivity set to %f" % tau_o)
        self.logger.info("transmitivity set to %f" % tau)

        # 10. Calculate daily photosynth. photon flux density (ppfd_d), mol/m^2
        ppfd_d = (1.0e-6)*kfFEC*(1.0 - kalb_vis)*tau*ra_d
        self.ppfd_d = ppfd_d
        self.logger.info("daily PPFD set to %f mol/m^2" % ppfd_d)

        # 11. Estimate net longwave radiation (rnl), W/m^2-----------------
        ...
        # 12. Calculate variable substitute (rw), W/m^2
        rw = (1.0 - kalb_sw) * tau * kGsc * dr
        self.rw = rw
        self.logger.info("variable substitute, rw, set to %f" % rw)

        # 13. Calculate net radiation cross-over hour angle (hn), degrees--
        ...
        # 14. Calculate daytime net radiation (rn_d), J/m^2----------------
