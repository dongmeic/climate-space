...
png(file=paste(out,"na10km_v2_cru_4.00.2015.anm.tmp3d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(file=paste(out,"na10km_v2_cru_4.00.2013.anm.tmp4d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.00.2015.abs.tmp3d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.00.2015.abs.tmp4d.png",sep=""))
...
ptm <- proc.time() # timer
absfile <- "na10km_v2_cru_ts4.00.1901.2015.tmp.abs3d.nc"
...
ptm <- proc.time() # timer
absfile <- "na10km_v2_cru_ts4.00.1901.2015.tmp.abs4d.nc"
abs_ncfile <- paste(path,absfile,sep="")
