...
inpath <- "/gpfs/projects/gavingrp/dongmeic/daymet/bioclm_na"; setwd(inpath)
...
varnms <- c("Lcs", "maxAugT", "summerT40", "OptTsum", "AugMaxT", "maxT", "Ecs", "Ncs", "Acs", "drop0", "drop5", "drop10",
 						"drop15", "drop20", "drop20plus", "max.drop", "ddAugJul", "ddAugJun", "Oct20", "Oct30", "Oct40", "Jan20", 
 						"Jan30", "Jan40", "Mar20", "Mar30", "Mar40", "winter20", "winter30", "winter40", "OctMin", "JanMin", "MarMin",
 						 "winterMin", "minT", "cv.gsp", "wd", "vpd", "mi", "cwd", "pt.coef")
...
varlnms <- c("late cold snap occurring between March through mid-April",
             ...
             "number of days with optimum summer temperatures 18–30 °C",
             "maximum daily maximum temperature in August",
             "maximum daily maximum temperature from August to July",
             ...
             "number of days with minimum temperatures at or below -20 °C during October",
             "number of days with minimum temperatures at or below -30 °C during October",
             "number of days with minimum temperatures at or below -40 °C during October",
             "number of days with minimum temperatures at or below -20 °C during January",
             "number of days with minimum temperatures at or below -30 °C during January",
             "number of days with minimum temperatures at or below -40 °C during January",
             "number of days with minimum temperatures at or below -20 °C during March",
             "number of days with minimum temperatures at or below -30 °C during March",
             "number of days with minimum temperatures at or below -40 °C during March",
             ...
             "minimum daily minimum temperature during October",
             "minimum daily minimum temperature during January",
             "minimum daily minimum temperature during March",
             "minimum daily minimum temperature during winter",
             "minimum daily minimum temperature from August to July",
             "coefficient of variation of growing-season precipitation",
             "the yearly sum of rainfall minus evapotranspiration in months with mean air temperature > 0 °C",
             "average monthly vapor pressure deficit in current and previous 5 growing seasons",
             "the ratio of annual precipitation to annual potential evapotranspiration",
             "cumulative climatic water deficit in current and previous 5 growing seasons",
             "the ratio of actual evapotranspiration to equilibrium evapotranspiration")
             
dunits <- c("binary", rep("day", 3), rep("°C", 2), "binary", "one", rep("day", 7), "°C",rep("DD", 2), 
						rep("day", 12), rep("°C", 5), "", "mm", "pascal", "", "mm", "")
            
dim1 <- 277910; dim2 <- nt
na.shp <- readOGR(dsn="/gpfs/projects/gavingrp/dongmeic/beetle/shapefiles", layer = "daymet_na10km")

print("writing 3D netCDF files")
ptm <- proc.time()
foreach(k = 1:nvar)%dopar%{
  indata1 <- read.csv(paste0("dm_bioclm_var_",years[1],"_na.csv"))
  indata2 <- read.csv(paste0("dm_bioclm_var_",years[1],"_na_wd.csv"))
  indata <- cbind(indata1, indata2)
  df <- data.frame(indata[,varnms[k]])
  colnames(df) <- years[1]
  for(i in 2:nt){
    indata1 <- read.csv(paste0("dm_bioclm_var_",years[i],"_na.csv"))
    indata2 <- read.csv(paste0("dm_bioclm_var_",years[i],"_na_wd.csv"))
    indata <- cbind(indata1, indata2)
  ...
  na_df[na.shp@data[,"seq_1_leng"],] <- df
  
  m <- rep(1:nt,each=dim1)
  temp_array <- array(fillvalue, dim=c(nx,ny,nt))
  temp_array[cbind(j2,k2,m)] <- as.matrix(na_df)
  ncfname <- paste0(ncpath,"ts/var/daily/na10km_v2_daymet_na_",varnms[k],"_1996.2015.3d.nc")
  dname <- varnms[k]
  dlname <- varlnms[k]
  dunit <- dunits[k]
  ...
  ncatt_put(ncout,0,"title","daymet values on the na10km_v2 10-km Grid")
  ncatt_put(ncout,0,"institution","Dept. Geography; Univ_ Oregon")
  ncatt_put(ncout,0,"source","generated by daymet_bioclim_ncfiles_na.R")
