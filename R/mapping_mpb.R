...
corehost_2 <- readOGR(dsn=hostpath, layer="core_host_without_jack_dissolve")
corehost_2 <- spTransform(corehost_2, na10km_crs)

roi <- readOGR(dsn="/gpfs/projects/gavingrp/dongmeic/beetle/shapefiles", layer="na10km_roi")
roi <- spTransform(roi, na10km_crs)
...
legend(-5500000, -3400000, bty="n", pch=0, col=rgb(0,0.6,0,1), cex=1.2, legend="Host range")
dev.off()

png(file=paste0(out,"map_of_mpb_2.png"),width=9, height=8, units="in", res=300)
par(mar=c(0,0,0,0))
plot(bb_shp, col="gray95")
points(shade_pixels, pch=19, cex=0.1, col=shade_colnum)
#title(paste("Mountain pine beetle presence in North America (1997 - 2016)"),line=0, cex.main=1.5)
...
plot(na_shp, add=TRUE)
#plot(coast_shp, lwd=0.3, add=TRUE)
plot(corehost_2,bord=rgb(0,0.6,0,1), add=TRUE)
df <- csvin_btl[,c("x","y","prs_1997")]
coordinates(df) <- c("x","y")
points2grid(df)
btl_pixels <- as(df, "SpatialPixelsDataFrame")
points(btl_pixels[btl_pixels$prs_1997==1,], pch=19, cex=0.1, col=rgb(0.8,0,0,0.2))
...
png(file=paste0(out,"maps_of_mpb_roi.png"),width=7, height=7, units="in", res=300)
par(mfrow=c(1,2),mar=c(0.5,0.5,1.5,0))
plot(roi, col='white')
points(shade_pixels, pch=19, cex=0.1, col=shade_colnum)
title("Outbreak in 1997",line=0, cex.main=1.5)
plot(can_shp, lwd=0.4, col="gray65", add=TRUE)
plot(us_shp, lwd=0.4, col="gray65", add=TRUE)
plot(lrglakes_shp, lwd=0.2, bor="black", col="lightblue", add=TRUE)
plot(corehost,bord=rgb(0,0.6,0,0.5), add=TRUE)
df <- csvin_btl[,c("x","y","expand","prs_1997")]
coordinates(df) <- c("x","y")
points2grid(df)
btl_pixels <- as(df, "SpatialPixelsDataFrame")
points(btl_pixels[btl_pixels$prs_1997==1,], pch=19, cex=0.1, col=rgb(0.8,0,0,0.5))
plot(roi, col='white')
points(shade_pixels, pch=19, cex=0.1, col=shade_colnum)
title("Outbreak after 1997",line=0, cex.main=1.5)
plot(can_shp, lwd=0.4, col="gray65", add=TRUE)
plot(us_shp, lwd=0.4, col="gray65", add=TRUE)
plot(lrglakes_shp, lwd=0.2, bor="black", col="lightblue", add=TRUE)
plot(corehost,bord=rgb(0,0.6,0,0.5), add=TRUE)
points(btl_pixels[btl_pixels$expand==1,], pch=19, cex=0.1, col=rgb(0.8,0,0,0.5))
points(btl_pixels[btl_pixels$prs_1997==1,], pch=19, cex=0.1, col=rgb(0.8,0,0,0.5))
dev.off()
