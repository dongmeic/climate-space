...
foreach(yr=1997:2016)%dopar%{
  ...
  points(btl_pixels[btl_pixels@data[,paste0("prs_", yr)]==1,], pch=19, cex=0.09, col=rgb(1,0,0,0.1))
  plot(bb_shp, add=TRUE)
  dev.off()
  print(paste("mapping mpb in",yr,"done!"))
}
...
points2grid(df)
btl_pixels <- as(df, "SpatialPixelsDataFrame")
points(btl_pixels[btl_pixels$allyears==1,], pch=19, cex=0.09, col=rgb(1,0,0,0.1))
plot(bb_shp, add=TRUE)
dev.off()
