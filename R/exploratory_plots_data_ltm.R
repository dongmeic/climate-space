...
# Exploratory data analysis: generate a datafile; long-term means are retrieved from 1901-2015
# Modified from exploratory_plots_data.R
...
year1 <- 1901; year2 <- 2015; year3 <- 1998; year4 <- 1999; year5 <- 2014
nyrs.t <- year5-year4+1;
nyrs.p <- year5-year3+1;
#win <- 1; # a window to examine mean annual temperature and precipitation
...
print(ncin_tmp)
dname <- "tmp"
tmp_ltm <- ncvar_get(ncin_tmp,dname)
dim(tmp_ltm)
...
write.csv(df.tmp, paste0(out, "climatic_changes_data_tmp_2.csv"))
write.csv(df.tstd, paste0(out, "climatic_changes_data_tstd_2.csv"))
write.csv(df.pre, paste0(out, "climatic_changes_data_pre_2.csv"))
write.csv(df.pstd, paste0(out, "climatic_changes_data_pstd_2.csv"))