...
btl_5km <- readOGR(dsn=path, layer="na_beetle_presence_allyears_5km", stringsAsFactors = FALSE)
...
# mpb
ncfname <- paste(ncpath,"prs/na10km_v2_mpb_presence_5km.nc", sep="")
dname <- "mpb_prs"
...
indata <- cbind(na10km,as.data.frame(btl_5km[,4:24])[1:21])
str(indata)
print("done!")
...
temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,8:27])
print("done!")
...
ncatt_put(ncout,0,"source","generated by na10km_v2_ncfiles_3d_btl_5km.R")
