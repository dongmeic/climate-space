...
crufile <- "cru_ts4.00.1901.2015.tmp.anm3d.nc"
...
napath <- "/home2/dongmeic/beetle/ncfiles/na10km_v2/ltm/"
nafile <- "na10km_v2.nc"
...
targpath <- "/home2/dongmeic/beetle/csvfiles/"
...
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(paste(out,"cru_ts4.00.tmp.interp.2015.12.3d.png", sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(paste(out,"cru_ts4.00.tmp.nonmiss.2015.12.3d.png", sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(paste(out,"cru_ts4.00.tmp.near.2015.12.3d.png", sep=""))
...
ptm <- proc.time() # timer
interppath <- "/home2/dongmeic/beetle/ncfiles/na10km_v2/ts/"
interpfile <- "na10km_v2_cru_ts4.00.1901.2015.tmp.anm3d.nc"
...
ncatt_put(ncout,0,"title","CRU CL 2.0 anomalies interpolated onto the na10km_v2 10-km Grid")
ncatt_put(ncout,0,"institution","Dept. Geography; Univ_ Oregon")
ncatt_put(ncout,0,"source","generated by cru_ts_4.00_regrid_anomalies.R")
...
ptm <- proc.time() # timer
interpfile <- "na10km_v2_cru_ts4.00.1901.2015.tmp.anm4d.nc"
interp_ncfile <- paste(interppath,interpfile,sep="")

# define dimensions
xdim <- ncdim_def("x",units="m",longname="x coordinate of projection",as.double(x))
ydim <- ncdim_def("y",units="m",longname="y coordinate of projection",as.double(y))
year <- seq(1901,2014, by=1)
