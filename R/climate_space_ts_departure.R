...
foreach(i=1:length(vargrp1))%dopar%{
  ...
  climate.space <- function(j){
    ...
    sdsum1 <- length(na.omit(df[df$prs=="mpb",]$v2))
    sdsum <- length(na.omit(df[df$prs=="mpb",]$v1))
    ...
    p <- qplot(v1, v2, data=df, color=factor(prs), alpha=I(0.7), xlab = paste(vargrp1[i], "(SD)"), ylab = paste(vargrp2[i], "(SD)"), main = years[j])+xlim(-5,5)+ylim(-5,5)
