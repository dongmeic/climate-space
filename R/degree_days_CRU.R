...
ptm <- proc.time()
for(y in 1:(nt-1)){
	indata1 <- read.csv(paste0("na10km_v2_monthly_mean_",years[y],".csv"))
	indata2 <- read.csv(paste0("na10km_v2_monthly_mean_",years[y+1],".csv"))
	indata <- rbind(indata1, indata2)
	
	y2.leap <- is.leap.year(y+1)
  ...
  aug.jul.range <- AUG:(AUG + YEAR - 1)
	...
	write.csv(df, paste0("daily_climate/CRU/degree_days_CRU_",years[y+1],".csv"), row.names = FALSE)  
}
proc.time() - ptm
print("all done!")