...
out <- "/gpfs/projects/gavingrp/dongmeic/beetle/output/20180613/"
...
png(file=paste(out,"na10km_v2_Pmean_tree_",end_year,".png",sep=""))
...
options(bitmapType='cairo')
png(file=paste(out,"na10km_v2_summerP0_std_beetle_",end_year,".png",sep=""))
...
foreach(i=1:length(dnames)) %dopar%{
	...
	history <- paste("D. Chen", date(), sep=", ")
	ncatt_put(ncout,0,"history",history)
	ncatt_put(ncout,0,"base_period","1903-2016")
	ncatt_put(ncout,0,"Conventions","CF-1_6")
...
history <- paste("D. Chen", date(), sep=", ")
ncatt_put(ncout,0,"history",history)
ncatt_put(ncout,0,"base_period","1907-2016")
ncatt_put(ncout,0,"Conventions","CF-1_6")
