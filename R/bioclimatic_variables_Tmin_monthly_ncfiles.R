...
for (k in 1:nt){
	...
	Tmin_3d <- apply(abind(tmn[,,11:12,k],tmn[,,1:3,(k+1)],along=3),c(1,2),mean)
	print(k)
}
for (k in 1:(nt+1)){
...
for (yr in (nt-nyr+1):nt){
	...
	if (btlyr == 1){
		...
		winterTmin_std <- abind(winterTmin_std_slice, winterTmin_std_vgt, winterTmin_std_btl, along=3)
		OctTmin_std <- abind(OctTmin_std_slice, OctTmin_std_vgt, OctTmin_std_btl, along=3)
		Tmin_std <- abind(Tmin_std_slice, Tmin_std_vgt, Tmin_std_btl, along=3)
	}else{
		print(paste0("start to reshape 2d to 3d in year ", years[btlyr]))
		...
		winterTmin_std <- abind(winterTmin_std, winterTmin_std_slice, winterTmin_std_vgt, winterTmin_std_btl, along=3)
		OctTmin_std <- abind(OctTmin_std, OctTmin_std_slice, OctTmin_std_vgt, OctTmin_std_btl, along=3)
		Tmin_std <- abind(Tmin_std, Tmin_std_slice, Tmin_std_vgt, Tmin_std_btl, along=3)
	}
...
winterTmin_std_4d <- array(winterTmin_std, dim=c(nx,ny,nv,nyr)) 
OctTmin_std_4d <- array(OctTmin_std, dim=c(nx,ny,nv,nyr)) 
Tmin_std_4d <- array(Tmin_std, dim=c(nx,ny,nv,nyr)) 
...
foreach(i=1:6) %dopar%{
	...
	ncatt_put(ncout,0,"base_period","1997-2015")
...
foreach(i=1:3) %dopar%{
	...
	history <- paste("D. Chen", date(), sep=", ")
	ncatt_put(ncout,0,"history",history)
	ncatt_put(ncout,0,"base_period","1901-2015")
	ncatt_put(ncout,0,"Conventions","CF-1_6")
...
for (yr in (nt-nyr+2):(nt+1)){
	...
	if (btlyr == 1){
		...
		JanTmin_std <- abind(JanTmin_std_slice, JanTmin_std_vgt, JanTmin_std_btl, along=3)
		MarTmin_std <- abind(MarTmin_std_slice, MarTmin_std_vgt, MarTmin_std_btl, along=3)
	}else{
		print(paste0("start to reshape 2d to 3d in year ", years[btlyr]))
		...
		JanTmin_std <- abind(JanTmin_std, JanTmin_std_slice, JanTmin_std_vgt, JanTmin_std_btl, along=3)
		MarTmin_std <- abind(MarTmin_std, MarTmin_std_slice, MarTmin_std_vgt, MarTmin_std_btl, along=3)
	}
	print(paste0(years[btlyr], " is done!"))
}
proc.time() - ptm
...
JanTmin_std_4d <- array(JanTmin_std, dim=c(nx,ny,nv,nyr)) 
MarTmin_std_4d <- array(MarTmin_std, dim=c(nx,ny,nv,nyr)) 
