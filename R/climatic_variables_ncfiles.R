...
out <- "/home2/dongmeic/beetle/output/20170727/"
...
year1 <- 1901; year2 <- 1998; year3 <- 1999; year4 <- 2014; year5 <- 2015

# subscripts of year 1998 and year 1999
n1 <- year2-year1+1; nyr.p <- year4 - year2 + 1 
n2 <- year3-year1+1; nyr.t <- year4 - year3 + 1
...
print(ncin)
dname <- "tmp"
tmp_ltm <- ncvar_get(ncin,dname)
dim(tmp_ltm)
tmp <- ncvar_get(ncin,dname,start=c(x=1,y=1,month=1,year=n2),count=c(x=1078, y=900, month=12, year=nyr.t))
dim(tmp)
...
ncfile_tmn_ltm <- "na10km_v2_cru_ts4.00.1901.2015.tmn.4d.nc"
...
ncfile_tmx_ltm <- "na10km_v2_cru_ts4.00.1901.2015.tmx.4d.nc"
...
print(ncin_pre)
dname <- "pre"
pre_ltm <- ncvar_get(ncin_pre,dname)
dim(pre_ltm)
pre <- ncvar_get(ncin_pre,dname,start=c(x=1,y=1,month=1,year=n1),count=c(x=1078,y=900, month=12, year=nyr.p))
dim(pre)
nc_close(ncin_pre)
...
for (yr in 1:nyr){
	...
	# 11. precipitation from June to August the previous year
	...
	# 13. precipitation from October to September the previous year
...
foreach(i=1:30) %dopar%{
	...
	ncatt_put(ncout,0,"source","generated by climatic_variables_ncfiles.R")
