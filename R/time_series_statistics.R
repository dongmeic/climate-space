...
# mpb
ncfname <- paste(ncpath,"prs/time_series_presence_alteration.nc", sep="")
csvfile <- "ts_beetle_presence_statistics.csv"
dname <- "prs_alt"
dlname <- "Mountain pine beetle presence alteration"
dunits <- "type"
...
temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,21:39])
print("done!")
...
for (i in yr.runs){
  ...
  p1 <- 40 + nt
  temp_array <- array(fillvalue, dim = c(nx,ny,nt))
  temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,:])  
  ncvar_put(ncout, var_def, temp_array)
}
print("done!")
dname <- "chosts_mpb_prs"
dlname <- "Mountain var beetle presence in core hosts"
