...
temp_array <- array(fillvalue, dim = c(nx,ny,nt))
temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,40:58]) 
ncvar_put(ncout, var_def, temp_array)
nc_close(ncout)
print("done!")
...
for (i in yr.runs[-1]){
  ...
  ncout <- nc_create(ncfname,list(lon_def,lat_def,var_def,proj_def),force_v4=TRUE, verbose=FALSE)
  nt1 <- length(years)+1-t
  nt2 <- length(years)+1-i
  p <- p + nt1 + 1 
  temp_array <- array(fillvalue, dim = c(nx,ny,nt))
  temp_array[cbind(j2,k2,m)] <- as.matrix(indata[1:nobs,p:(p-1+nt2)])
  t <- i
  # put variables
  ncvar_put(ncout,lon_def,lon)
  ncvar_put(ncout,lat_def,lat)
