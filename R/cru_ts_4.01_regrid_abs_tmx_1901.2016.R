...
ncinfile <- "na10km_v2_cru_ts4.01.1901.2016.tmx.anm3d.nc"
...
png(file=paste(out,"na10km_v2_cru_4.01.2016.anm.tmx3d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(file=paste(out,"na10km_v2_cru_4.01.2014.anm.tmx4d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()

# read long-term mean data
ltmxath <- "/gpfs/projects/gavingrp/dongmeic/beetle/ncfiles/na10km_v2/ltm/"
ltmxc <- "na10km_v2_tmx.nc"
ltmxcfile <- paste(ltmxath,ltmxc,sep="")
ltmxcin <- nc_open(ltmxcfile)
print(ltmxcin)

ltm <- ncvar_get(ltmxcin,"tmx")
dim(ltm)
...
png(file=paste(out,"na10km_ltm.tmx3d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.01.2016.abs.tmx3d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.01.2016.abs.tmx4d.png",sep=""))
...
ptm <- proc.time() # timer
absfile <- "na10km_v2_cru_ts4.01.1901.2016.tmx.abs3d.nc"
...
ptm <- proc.time() # timer
absfile <- "na10km_v2_cru_ts4.01.1901.2016.tmx.abs4d.nc"
abs_ncfile <- paste(path,absfile,sep="")
