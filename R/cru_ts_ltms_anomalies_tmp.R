...
ncinfile <- "cru_ts4.00.1901.2015.tmp.dat.nc"
...
png(paste(out,"cru_ts4.00.tmp.2012.12.3d.png", sep=""))
...
grid <- expand.grid(lon=lon, lat=lat)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(paste(out,"cru_ts4.00.tmp.2013.06.4d.png", sep=""))
...
ncfname <- paste(ncoutpath,"cru_ts4.00.1961-1990.tmp.ltm.nc",sep="")
...
png(paste(out,"cru_ts4.00.tmp.anm.2015.12.3d.png", sep=""))
levelplot(var_slice_3d ~ lon * lat, data=grid, at=cutpts, cuts=11, pretty=T, 
  col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
ncfname <- paste(ncoutpath,"cru_ts4.00.1901.2015.tmp.anm3d.nc",sep="")
...
# add global attributes
ncatt_put(ncout,0,"title","CRU TS 4.00 2m air temperature anomalies")
ncatt_put(ncout,0,"institution","Dept. Geography; Univ_ Oregon")
ncatt_put(ncout,0,"source",ncinfile)
history <- paste("D. Chen", date(), sep=", ")
ncatt_put(ncout,0,"history",history)
...
# create netCDF file and put arrays
ncfname <- paste(ncoutpath,"cru_ts4.00.1901.2015.tmp.anm4d.nc",sep="")
