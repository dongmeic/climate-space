...
ncinfile <- "na10km_v2_cru_ts4.01.1901.2016.tmp.anm3d.nc"
...
png(file=paste(out,"na10km_v2_cru_4.01.",end_year,".anm.tmp3d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(file=paste(out,"na10km_v2_cru_4.01.",end_year,".anm.tmp4d.png",sep=""))
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.01.",end_year,".abs.tmp3d.png",sep=""))
levelplot(var_slice_3d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(x=x, y=y)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(file=paste(out,"na10km_v2_cru_4.01.",end_year,".abs.tmp4d.png",sep=""))
levelplot(var_slice_4d ~ x * y, data=grid, at=cutpts, cuts=11, pretty=T, 
          col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
ptm <- proc.time() # timer
absfile <- paste0("na10km_v2_cru_ts4.01.",start_year,".",end_year,".tmp.abs3d.nc")
...
ptm <- proc.time() # timer
absfile <- paste0("na10km_v2_cru_ts4.01.",start_year,".",end_year,".tmp.abs4d.nc")
abs_ncfile <- paste(path,absfile,sep="")
