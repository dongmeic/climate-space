...
ncinfile <- "cru_ts4.01.1901.2016.tmn.dat.nc"
...
png(paste(out,"cru_ts4.01.tmn.2012.12.3d.png", sep=""))
...
grid <- expand.grid(lon=lon, lat=lat)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(paste(out,"cru_ts4.01.tmn.2014.06.4d.png", sep=""))
...
grid <- expand.grid(lon=lon, lat=lat)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(paste(out,"landmask_tmn.png", sep=""))
...
grid <- expand.grid(lon=lon, lat=lat)
cutpts <- c(-50,-40,-30,-20,-10,0,10,20,30,40,50)
png(paste(out,"cru_ts4.01.tmn.ltm.12.3d.png", sep=""))
levelplot(var_slice_3d ~ lon * lat, data=grid, at=cutpts, cuts=11, pretty=T, 
  col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
var_def <- ncvar_def("tmn_ltm","deg_C",list(londim,latdim,tdim),fillvalue,dlname,prec="single")

# create netCDF file and put array
ncfname <- paste(ncoutpath,"cru_ts4.01.1961-1990.tmn.ltm.nc",sep="")
...
png(paste(out,"cru_ts4.01.tmn.anm.2015.12.3d.png", sep=""))
levelplot(var_slice_3d ~ lon * lat, data=grid, at=cutpts, cuts=11, pretty=T, 
  col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
grid <- expand.grid(lon=lon, lat=lat)
cutpts <- c(-10,-5,-2,-1,-.5,0,.5,1,2,5,10)
png(paste(out,"cru_ts4.01.tmn.anm.2016.06.4d.png", sep=""))
levelplot(var_slice_4d ~ lon * lat, data=grid, at=cutpts, cuts=11, pretty=T, 
  col.regions=(rev(brewer.pal(10,"RdBu"))))
dev.off()
...
tmn_def <- ncvar_def("tmn_anm","deg_C",list(londim,latdim,timedim),fillvalue,dlname,prec="single")

# create netCDF file and put arrays
ncfname <- paste(ncoutpath,"cru_ts4.01.1901.2016.tmn.anm3d.nc",sep="")
ncout <- nc_create(ncfname,tmn_def,force_v4=T)

# put variables
ncvar_put(ncout,tmn_def,var3d)

# put additional attributes into dimension and data variables
ncatt_put(ncout,"time","axis","T")
ncatt_put(ncout,"time","calendar","standard")
ncatt_put(ncout,"tmn_anm","comment","anomalies, differences from 1961-1990 base period long-term means")
...
# define variables
fillvalue <- 1e32
dlname <- "2m air temperature minimum anomalies"
tmn.def <- ncvar_def("tmn_anm","deg_C",list(londim,latdim,monthdim,yeardim),fillvalue,dlname,prec="single")

# create netCDF file and put arrays
ncfname <- paste(ncoutpath,"cru_ts4.01.1901.2016.tmn.anm4d.nc",sep="")
ncout <- nc_create(ncfname,tmn.def,force_v4=T)

# put variables
ncvar_put(ncout,tmn.def,var4d)

# put additional attributes into dimension and data variables
ncatt_put(ncout,"tmn_anm","comment","anomalies, differences from 1961-1990 base period long-term means")

# add global attributes
ncatt_put(ncout,0,"title","CRU TS 4.01 2m air temperature minimum anomalies")
ncatt_put(ncout,0,"institution","Dept. Geography; Univ_ Oregon")
ncatt_put(ncout,0,"source",ncinfile)
history <- paste("D. Chen", date(), sep=", ")
ncatt_put(ncout,0,"history",history)
