...
# Get the means of minimum witer temperature and maximum August temeprature
# Did not change the output file names, the previous copies are saved in the local drive /Users/dongmeichen/Documents/beetle/20170717
...
mpb_ncfile <- "na10km_v2_mpb_presence.nc"
ncin_mpb <- nc_open(paste(prs_path,mpb_ncfile, sep=""))
print(ncin_mpb)
mpb <- ncvar_get(ncin_mpb,"mpb_prs")
...
for (yr in 2:nyr){
  ...
  if (years[yr] >= 2000){
  	btl_slice <- mpb[,,which(years2==years[yr])]
  }else{
  	btl_slice <- btl
  }
  ...
  df_clm.tmp <- data.frame(clm_mat,clm_mtaa,clm_mta,clm_ntw,clm_nto,clm_ntj,clm_ntm,clm_xta,clm_prs,clm_yr=rep(toString(years[yr-1]),length(clm_mat)))
  df_clm.pre <- data.frame(clm_map,clm_cpja,clm_pja,clm_cpos,clm_pos,clm_gsp,clm_vgp=c(clm_vgp,rep(NA,(len.clm-length(clm_vgp)))),clm_prs,clm_yr=rep(toString(years[yr-1]),length(clm_map)))
  ...
  df_btl.tmp <- data.frame(btl_mat,btl_mtaa,btl_mta,btl_ntw,btl_nto,btl_ntj,btl_ntm,btl_xta,btl_prs,btl_yr=rep(toString(years[yr-1]),length(btl_mat)))
  df_btl.pre <- data.frame(btl_map,btl_cpja,btl_pja,btl_cpos,btl_pos,btl_gsp,btl_vgp=c(btl_vgp,rep(NA,(len.btl-length(btl_vgp)))),btl_prs,btl_yr=rep(toString(years[yr-1]),length(btl_map)))
